...
assert.match(y,/timeout-minutes:\s*15/);
assert.match(y,/\^\[a-z0-9\]\{32\}\$/,"strict FRED secret format guard missing");
assert.match(y,/npm run fred-smoke/,"live FRED smoke test step missing");
...
assert.match(pkg.scripts?.preflight||"",/preflight\.mjs/,"live preflight command missing");
...
assert.ok(existsSync(new URL("./preflight.mjs",import.meta.url)),"preflight script missing");
...
assert.match(readme,/23 \*\/8/,"README schedule does not match workflow");
assert.doesNotMatch(readme,/23 \*\/4|раз в четыре часа/,"stale four-hour documentation remains");
assert.match(y,/REQUIRE_COMPLETE:\s*["']?1["']?/,"live candidate must require both regimes");
