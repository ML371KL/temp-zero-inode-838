...
try{
  ...
  assert.equal(d.source_urls.length,3,"all derivative source links exposed");
  assert.equal(d.data.funding.length,3);
  ...
  assert.deepEqual(spot.data,{coinbase:100050,kraken:100000,bitstamp:100060,okx:100020,bybit:100010,kraken_usdt:100030,coinbase_usdt:100040});
  ...
  assert.equal(partial.data.funding.length,3,"funding/OI survives an options failure");
  ...
  assert.equal(quoteGroupPrices(geo.data,"USD").length,3,"USD group intact (Coinbase/Kraken/Bitstamp)");
