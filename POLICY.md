...
including SHA-256 digests of the economically material scoring, regime and hysteresis code.
...
the same vintage or changes schema, the event is recorded rather than silently treated as the value
that was known earlier.

Policy v1 is monitored prospectively against the previous policy in shadow mode plus buy & hold,
fixed 50% BTC, cash and a simple trend/25%-volatility benchmark. Results include cash yield from
FRED DTB3, 10 bps per full turnover, turnover, net return, drawdown, volatility and excess Sharpe.
...
possible only after at least 365 days and two target changes, and only if all three conditions hold:
...
Operational failures—stale public snapshots, policy-hash mismatch, an unreproducible decision or a
missing actionable target—pause the displayed action and open/update a GitHub incident through the
